...
typedef struct vertex_text_coor {
    double u = -1;
    double v = -1;
} vertex_text_coor;
...
typedef struct light { int s = -1; double arr[LIGHT_ARR_SIZE];
                        double angle = 0; /*highlight angle*/
                        double radius = 0;} light;
...
class FileStream {
    public:
        ...
        //object array;
        object objects[255];
        int obj_count = 0;      //how many elem in objects arr
        int get_obj_count = 0;  //how many elems are exported
        //light array;
        light lights[255];
        int light_count = 0;    //how many elem in lights arr
        ...
        std::vector<triangle_mesh> tri_meshes;
        std::vector<face> fs;
    ...
    private:   
        ...
        std::string KEYWORDS[NUM_OF_KEYWORDS];
